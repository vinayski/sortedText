<script>
  ...
  function natsort(str) {
    var arr = str.split(" ");
    arr = arr.map(function(str) {
      return str.charAt(0).toUpperCase() + str.slice(1);
    });
    var sorted = arr.sort(naturalSort);
    return sorted.join(" ");
  }
...
<h1>Input to be sorted</h1>

<div id="text">
  <input bind:value={name} placeholder="enter your string to be sorted" />
</div>

<h1>Sorted Text</h1>
<p>{sortString(name)}</p>

<h1>Natural Sorted Text with First Letter Capitalized</h1>
<p>{natsort(name)}</p>
